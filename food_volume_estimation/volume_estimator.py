...
class DensityDatabase():
    ...
    def __init__(self, db_path):
        ...
        if os.path.exists(db_path):
            ...
            self.density_database = pd.read_excel(db_path, sheet_name='Density DB')
        else:
            # Read density database from Google Sheets URL
            sheet = 'Sheet1'
            url = 'https://docs.google.com/spreadsheets/d/{0}/gviz/tq?tqx=out:csv&sheet={1}'.format(
                db_path, sheet)
            self.density_database = pd.read_csv(url, header=None)
    ...
    def query(self, food):
        ...
        try:
            ...
            # Return default: Name, Density=1, then 0s for others
            # Assuming columns: Food, Density, kcal, protein, carbs, fat, sugar
            # We want to return a list that matches the length of columns if possible, 
            # but 'None', 1 is the old behavior. 
            # Let's return a safe default with 0s for nutrition.
            # safe length = len(self.density_database.columns)
            # but if it fails to init, we might not know cols.
            # Assuming standard structure now.
            return ['None', 1.0, 0, 0, 0, 0, 0]
...
class VolumeEstimator():
    ...
    def __init__(self, arg_init=True):
        ...
        if not arg_init:
            ...
            self.monovideo.load_weights(self.args.depth_model_weights, by_name=True)
            ...
            self.segmentator = FoodSegmentator(self.args.segmentation_weights, self.args.segmentation_config)
    ...
    def __parse_args(self):
        ...
        parser.add_argument('--segmentation_weights', type=str,
                            help='Food segmentation model weights (.onnx).',
                            metavar='/path/to/weights.onnx',
                            required=True)
        parser.add_argument('--segmentation_config', type=str,
                            help='Food segmentation model config (.yaml).',
    ...
    def estimate_volume(self, input_image, fov=70,  plate_diameter_prior=0.3,
        ...
        plate_contour = (img * 255).astype(np.uint8)
        if any(x != 0 for x in ellipse_params_scaled):
            center = (int(ellipse_params_scaled[0]), int(ellipse_params_scaled[1]))
            axes = (int(ellipse_params_scaled[2]), int(ellipse_params_scaled[3]))
            angle = int(ellipse_params_scaled[4] * 180 / np.pi)
            cv2.ellipse(plate_contour, center, axes, angle, 0, 360, (0, 255, 0), 2)
        ...
        masks_array, class_names = self.segmentator.infer_masks(input_image)
            ...
            if (plot_results or plots_directory is not None):
                ...
                object_points_df = pd.DataFrame(
                    np.concatenate((object_points, object_colors), 
                                   axis=1),
                    columns=['x', 'y', 'z', 'red', 'green', 'blue'])
                object_points_filtered_df = pd.DataFrame(
                    np.concatenate((object_points_filtered, 
                ...
                if plots_directory is not None:
                    ...
                    # Save point clouds to disk using PyntCloud
                    #PyntCloud(non_object_points_df).to_file(
                    #    plots_directory + '/non_object_points.ply')
                    PyntCloud(object_points_df).to_file(
                        plots_directory + '/object_points.ply')
                    PyntCloud(object_points_filtered_df).to_file(
                        plots_directory + '/object_points_filtered.ply')
                    PyntCloud(plane_points_df).to_file(
                        plots_directory + '/plane_points.ply')
                    #PyntCloud(plane_points_transformed_df).to_file(
                    #    plots_directory + '/plane_points_transformed.ply')
                    #PyntCloud(object_points_transformed_df).to_file(
                    #    plots_directory + '/object_points_transformed.ply')

                # Estimate volume for points above the plane
                volume_points = object_points_transformed[
                    object_points_transformed[:,2] > 0]
                estimated_volume, simplices = pc_to_volume(volume_points, alpha=0.1)
                ...
                volume_points = object_points_transformed[
                    object_points_transformed[:,2] > 0]
                estimated_volume, _ = pc_to_volume(volume_points, alpha=0.1)
                # Store volume and class name
                current_class = class_names[k] if k < len(class_names) else 'unknown'
                estimated_volumes.append((estimated_volume, current_class))
